...
(function($) {
  ...
  // this allows you to specify callbacks as functions or strings (including CoffeeScript)
  function evfun(fun, hint) {
    var src = null    
    if (fun && fun.match && fun.match(/^function/)) {
      src = "var f = "+fun
    } else if (CoffeeScript && fun && fun.match && fun.match(/^\(.*\)[ ]*->.*/)) { //CoffeeScript header regexp
      try {
        src = CoffeeScript.compile("f = " + fun, {bare: true})
      } catch (e) {
        $.log("CoffeeScript compilation error", e)
        alert("CoffeeScript compilation error\n" + e)
      }
    }
    if (src) { 
      eval(src);
